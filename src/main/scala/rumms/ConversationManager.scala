...
final class ConversationManager {
	...
	def find(id:Predicate[ConversationId]):Iterable[Conversation]	= synchronized {
		entries filter { it => id(it.id) }
	}
	
	/** get a Conversation by id */
	def get(id:ConversationId):Option[Conversation] = synchronized {
		entries find { _.id == id }
	}
	
	/** insert a new Conversation */
	def put(conversation:Conversation):Unit = synchronized {
		conversation.touch()
		entries	= entries :+ conversation
	}
	
	/** get a Conversation by id and keep it alive */
	def use(id:ConversationId):Option[Conversation] = synchronized {
	...
	def expire():Iterable[Conversation] = synchronized {
