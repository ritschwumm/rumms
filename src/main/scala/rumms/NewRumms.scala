...
object NewRumms extends Logging {
	def create(configuration:RummsConfiguration):IoResource[NewRumms]	=
		for {
			rumms	<-	IoResource delay new NewRumms(configuration)
...
final class NewRumms(configuration:RummsConfiguration) { outer =>
